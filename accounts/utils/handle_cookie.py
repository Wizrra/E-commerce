...
def create_session_token(user_id):
    # Ensure user_id is JSON-serializable (UUID -> string)
    payload = {
        'user_id': str(user_id),
    }
...
def set_session_cookie(response, user_id):
    ...
    response.set_cookie(
            ...
            # samesite='Lax'  # Adjust as needed (e.g., 'Strict' or 'None' with secure=True)
        )
    return response

def set_access_refresh_tokens(response, user):
